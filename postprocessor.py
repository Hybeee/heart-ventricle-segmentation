...
def dummy_decider(nms_result: dict, data_dict: dict, thresholds_dir: str, polar_grad: np.ndarray):
    best_threshold = None
    best_score = -np.inf
...
def _get_input(input_dir):
    ...
    return score_data, data_dict, thresholds_dir, polar_grad

def postprocess(config, output_dir):

    score_data, data_dict, thresholds_dir, polar_grad = _get_input(output_dir)
